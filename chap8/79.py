...
class LogisticRegression:
    ...
    def calc_score(self, threshould=0.0):
        stemmer = PorterStemmer()
        cnt = 0
        correct_count = 0
                    ...
                    for w in words:
                        if not self.is_stop_words(w) and w:
                            try:
                                ...
                                continue
                    probability = (1.0 / (1.0 + np.exp(-sum([self.weights[f] for f in features]))) - threshould)
                    calc_polarity = 1.0 if probability > 0.5 else -1.0
...
lr.calc_weights()
for threshould in [1.0*t / 20 for t in xrange(10)]:
    print threshould, lr.calc_score(threshould)
